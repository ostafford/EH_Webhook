...
| `phoneNumber` (top-level) | — | `mobileNumber` | already E.164 (`+61…`) |
...
| — | — | `isPostalAddressSameAsResidential` | always `true` (no postal address in Connecteam) |
...
| TFN | 42923222 | `taxFileNumber` | string; keep leading zeros |
| Claim tax-free threshold? | 42923276 | `claimTaxFreeThreshold` | Yes → true |
| Australian resident for tax purposes? | 42923315 | resident scale / `isNonResident` | Yes → resident; No → foreign resident. Working-holiday-maker NOT modelled — manual |
| Have a HELP/STSL study debt? | 42923316 | `hasHelpDebt` (+ STSL) | Yes → true; verify exact EH field name in build |
...
| Super Fund USI | 42920803 | APRA fund USI / product code | **if USI present → APRA path** |
| Member Number | 42920804 | `superFund1_MemberNumber` | |
| Super Fund Name | 42920782 | `superFund1_FundName` | |
| Super Fund ABN | 42920783 | fund ABN | if USI blank + ABN present → **SMSF → do NOT sync super**, correction message: "SMSF super is set up manually by payroll" |
...
## Still to verify during the build spike

- Exact EH enum values: `gender`, residency/tax-scale fields, HELP vs STSL field names.
- Real EH `422` error body shape → drives the curated error→message map.
- Whether `user_updated` fires (and is signed) on custom-field edits — needs a live webhook.
- Whether an un-approve in Connecteam moves the assignment out of `status: completed`.
